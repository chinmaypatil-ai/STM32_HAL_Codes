...
void Stepper_SetSpeed(uint32_t hz)
{
    uint32_t arr = (10000000 / hz) - 1;     // adjust timer clock!
    __HAL_TIM_SET_AUTORELOAD(&htim3, arr);
}

void Stepper_MoveTo(int32_t steps)
{
    currentSteps = 0;
    targetSteps = abs(steps);

    Stepper_SetDir(steps > 0);
...
int main(void)
{
  ...
  Stepper_SetSpeed(500);     // 500 Hz

  Stepper_MoveTo(1000);     // forward
